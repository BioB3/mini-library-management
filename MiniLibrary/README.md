...
# Run the application
dotnet run --project src/MiniLibrary.Web

# Or run with Aspire (if using)
dotnet run --project src/MiniLibrary.AspireHost
```
...
```powershell
dotnet ef database update -c AppDbContext -p src/MiniLibrary.Web -s src/MiniLibrary.Web
dotnet run --project src/MiniLibrary.Web
```
